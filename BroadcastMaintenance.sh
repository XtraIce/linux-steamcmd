...
if $BROADCAST_MAINTENANCE; then
    ...
    if $(pgrep -f $TARGET_APP >/dev/null) ; then
        echo "Broadcast Server_will_be_shutdown_in_30_mins" | $ARRCON -S game_server && \
        echo "Broadcast for_scheduled_maintanence_-Red" | $ARRCON -S game_server
        if [ $? -eq 0 ]; then
            sleep 20m
            echo "Broadcast Server_will_be_shutdown_in_10_mins" | $ARRCON -S game_server && \
            echo "Broadcast for_scheduled_maintanence" | $ARRCON -S game_server
            sleep 9m 30s
