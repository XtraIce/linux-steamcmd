#!/bin/bash
DATA_DIR=${DATA_DIR:-"/home/steam/serverdata"}
...
if $(pgrep -f $TARGET_APP >/dev/null) ; then
	...
	echo "${RCON_BROADCAST_CMD} Server_will_be_shutdown_in_30_mins" | $ARRCON -S game_server && echo "${RCON_BROADCAST_CMD} for_scheduled_maintanence_-Red" | $ARRCON -S game_server
	if [ $? -eq 0 ]; then
		sleep 20m
		echo "${RCON_BROADCAST_CMD} Server_will_be_shutdown_in_10_mins" | $ARRCON -S game_server && echo "${RCON_BROADCAST_CMD} for_scheduled_maintanence" | $ARRCON -S game_server
		sleep 9m 30s
